  ...
  render() {
    ...
    return (
      <tr key={id} id={`file${id}`} className="user-table">
        <td>
          <img src={getIcon(isFile, ext)} alt={ext} width="24px" />
        </td>
        <td>
          <p>{id}</p>
        </td>
        ...
        <td className="number-column">
          <p className="number-column">{ext}</p>
        </td>
        <td className="number-column">
          <p
            style={{ minWidth: '110px', maxWidth: '110px', textAlign: 'left' }}
          >
            {lastModified}
          </p>
        </td>
        <td className="number-column" style={{ maxWidth: '75px' }}>
          {size}
        </td>
        <td style={{ textDecoration: 'none', textAlign: 'center' }}>
          {' '}
          <a href={`#file${dependency}`}>{dependency}</a>{' '}
        </td>
        <td>
          <button onClick={() => console.log(nombre, nivel)}>
            <i
              className={`fa fa-save`}
              style={{ fontSize: '26px', color: '#424242' }}
            ></i>
          </button>
        </td>
      </tr>
