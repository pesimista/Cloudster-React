...
const routesComponents = [<FolderIcon />, <PlayCircleFilledIcon />, <CloudUploadIcon />, <AccountCircleIcon />, <SettingsIcon />];
    ...
    */
    ...
    const uploadFile = (data) => {
        ...
        fetch(`/api/files/${globalState.folder}`, {
            ...
            update({ open: true, message: 'Archivo subido satisfactoriamente!', severity: 'success' });
        }).catch((mistake) => {
            console.log(
            ...
            >
                {
                    routes.reduce((prev, route, index) => {
                        ...
                        if (index !== 2) {
                            const fragment = (
                                <BottomNavigationAction key={index} className={classes.sectionMobile} to={route} component={reactLink} icon={routesComponents[index]} />
                            )
                            prev.push(fragment);
                        } else {
                            const fragment = (
                                <BottomNavigationAction key={index} component={Typography} className={classes.sectionMobile} onClick={() => handleUploadModal()} icon={routesComponents[index]} />
